...
model User {
  ...
  // Task relations - CORRECTED: Only these two
...
model Donor {
  ...
  organizationId String
  organization   Organization  @relation(fields: [organizationId], references: [id])
  files     File[]   // 👈 must exist if you include it
  ...
  tasks          Task[]              // Donor tasks relation
...
model ActivityFeed {
  ...
  // Who performed the action (optional - could be system)
  userId         String?
  user           User?         @relation(fields: [userId], references: [id])
  
  // What donor is involved (optional - for donor-related actions)
  donorId        String?
  donor          Donor?        @relation(fields: [donorId], references: [id])
  
  // What donation is involved (optional - for donation-related actions)
  donationId     String?
  donation       Donation?     @relation(fields: [donationId], references: [id])
  
  // Activity details
  action         String        // e.g., "DONATION_RECEIVED", "MEETING_SCHEDULED", "THANK_YOU_SENT"
  title          String        // Short title for display
  description    String?       // Longer description
  amount         Float?        // Amount if applicable
  
  // Metadata
  metadata       Json?         // Additional data like meeting time, note content, etc.
  priority       String        @default("NORMAL") // LOW, NORMAL, HIGH, URGENT
  isRead         Boolean       @default(false)
  
  // Timestamps
  createdAt      DateTime      @default(now())
  expiresAt      DateTime?     // When to auto-remove from feed
...
model DonorActivity {
  ...
  // Action details
  type           String        // DONATION, MEETING, COMMUNICATION, STATUS_CHANGE, NOTE_ADDED
  action         String        // More specific: "DONATION_RECEIVED", "MEETING_SCHEDULED", etc.
  ...
  relatedCommunication Communication? @relation(fields: [relatedCommunicationId], references: [id])
  ...
  @@index([organizationId, donorId, type])
}
...
model Task {
  ...
  // Task assignment relations - CORRECTED: Only these two
  assignedToId   String?
